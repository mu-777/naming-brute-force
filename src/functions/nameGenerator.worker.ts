...
self.onmessage = async (e: MessageEvent<{
  ...
  const candidateKanjiList: string[] = ((charCount - kanjiInput.length) === 1 && strokeCounts.length > 0 ?   // 2文字指定で、入力漢字が1つの場合 or 3文字指定で、入力漢字が2つの場合
    ...
    : Object.keys(kanjiDict)).filter(kanji => !excludedKanjiSet.has(kanji)).sort(() => Math.random() - 0.5);;

  if (candidateKanjiList.length === 0) {  // ありえないStrokeCountを指定されたときなど
    self.postMessage({ type: 'partial', results: [] });
  }
    ...
    if (isOnceLoop) {
      for (const kanji of candidateKanjiList) {
        if ((kanjiInput.length === 1 && kanji === kanjiInput[0])
          || (kanjiInput.length === 2 && (kanji === kanjiInput[0] || kanji === kanjiInput[1]))) {
          continue;
        }
